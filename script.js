...
if(carousel){
  ...
  let index=0,timer;
  slides.forEach((_,i)=>{const b=document.createElement('button');b.type='button';b.setAttribute('aria-label','Go to video '+(i+1));b.addEventListener('click',()=>go(i,true));dots.appendChild(b)});
  function setActive(){
    slides.forEach((s,i)=>s.classList.toggle('active',i===index));
    track.style.transform=`translate3d(-${index*100}%,0,0)`;
    [...dots.children].forEach((d,i)=>d.classList.toggle('on',i===index));
    slides.forEach((s,i)=>{const v=s.querySelector('video'); if(!v)return; if(i===index){v.muted=true;v.setAttribute('muted','');v.load(); const p=v.play(); if(p)p.catch(()=>{});} else {v.pause();}});
  }
  function go(i,manual=false){index=(i+slides.length)%slides.length;setActive();clearTimeout(timer);timer=setTimeout(()=>go(index+1),7000)}
  ...
  setActive();
  document.addEventListener('visibilitychange',()=>{if(document.hidden) clearTimeout(timer); else timer=setTimeout(()=>go(index+1),7000)});
}
